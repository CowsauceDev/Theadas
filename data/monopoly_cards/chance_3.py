...
class chance_3(trust.Card):
    def __init__(self, variant: trust.Variant):
        match variant:
            case trust.Variant.BASE: super().__init__("", "Advance to Ilinois Avenue. Collect $200 if you pass go.", "assets/card.png")
            case trust.Variant.HELLOPOLY: super().__init__("Enjoy the Clowns, Bitch", "Advance to Mammon's Theater. Collect $200 if you pass go.", "assets/card.png")

    def on_draw(self, player, game):
        if player.position > 24:
            player.cash += 200
            player.net_worth += 200
        player.move(game, position = 24)